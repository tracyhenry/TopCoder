...
LL mind[100][100], oneNeg[100][100];
LL minw[100][100], maxw[100][100];
LL ans[100][100], a[100][100];
...
long long NegativeGraphDiv1::findMin(int N, vector <int> from, vector <int> to, vector <int> weight, int charges) 
{
	...
	for (int i = 0; i < weight.size(); i ++)
	{
		...
		LL w = (LL) weight[i];
		//update
		minw[x][y] = min(minw[x][y], w);
		maxw[x][y] = max(maxw[x][y], w);	
	}
	
	//initialize mind[][]
	memcpy(mind, minw, sizeof(minw));
		
	//make mind using floyd
	for (int k = 1; k <= N; k ++)
		for (int i = 1; i <= N; i ++)
			for (int j = 1; j <= N; j ++)
				mind[i][j] = min(mind[i][j], mind[i][k] + mind[k][j]);
	
	//make oneNeg[][] using mind[][]
	for (int i = 1; i <= N; i ++)
		for (int j = 1; j <= N; j ++)
			oneNeg[i][j] = -maxw[i][j];
	for (int k = 1; k <= N; k ++)
		for (int i = 1; i <= N; i ++)
			for (int j = 1; j <= N; j ++)
				oneNeg[i][j] = min(oneNeg[i][j], oneNeg[i][k] + mind[k][j]);
	
	//matrix multiplication
	//oneNeg ^ charges
	memcpy(ans, mind, sizeof(mind));
	memcpy(a, oneNeg, sizeof(oneNeg));
	...
	while (charges > 0)
	{
		if (charges & 1)
		{
			...
			for (int i = 1; i <= N; i ++)
				for (int j = 1; j <= N; j ++)
					for (int k = 1; k <= N; k ++)
						tmp[i][j] = min(tmp[i][j], ans[i][k] + a[k][j]);
